...
class DiceLossOld(nn.Module):
    ...
    Shape:
        ...
        - Output: Scalar loss
    Examples:
from src.dice_loss_old import DiceLossOld 
import torch
loss = DiceLossOld(with_logits=True, ohem_ratio=0.1)
input = torch.FloatTensor([2, 1, 2, 2, 1]).view([-1,1])
...
output.backward()
    ...
    def _compute_dice_loss(self, flat_input, flat_target):
        ...
        if not self.square_denominator:
            ...
            loss = 1 - ((2 * interection + self.smooth) /
                        (torch.sum(torch.square(flat_input, ), -1) + torch.sum(
                            torch.square(flat_target), -1) + self.smooth))
    ...
    def _multiple_class(self, input, target, logits_size, mask=None):
        ...
        flat_input = torch.nn.Softmax(
            dim=1)(flat_input) if self.with_logits else flat_input
            ...
            for label_idx in range(logits_size):
                ...
                if keep_num > 0:
                    neg_scores = torch.masked_select(
                        flat_input, neg_example.view(-1, 1).bool()).view(
                            -1, logits_size)
